...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-3">
                ...
                <div class="px-6 py-4">
                    <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $admission->email ?? 'N/A' }}</dd>
                        </div>
                        <div class="sm:col-span-2">
        ...
        <div class="space-y-8">
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow">
                <div class="border-b border-gray-200 bg-gray-50 px-6 py-4">
                    <h3 class="text-lg font-medium text-gray-900">SMS Actions</h3>
                </div>
                <div class="px-6 py-4 space-y-2">
                    @if($admission->payment_status != 'paid')
                    <form action="{{ route('admin.admissions.resend-payment', $admission->id) }}" method="POST">
                        @csrf
                    ...
                    <div class="mt-2">
                        <label for="custom_message" class="block text-sm font-medium text-gray-700">Custom SMS Message</label>
                        <textarea id="custom_message" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gold focus:ring-gold"
                            placeholder="Type your custom SMS message here..."></textarea>
                        <button onclick="sendCustomSms()" class="mt-3 w-full rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Send Custom SMS
                        </button>
                        <div id="sms_response" class="mt-2 text-sm hidden"></div>
                    </div>
                </div>
            </div>
...
function sendCustomSms() {
    ...
    const responseDiv = document.getElementById('sms_response');
    
    if (!message.trim()) {
        responseDiv.className = 'mt-2 text-sm text-red-600';
        responseDiv.textContent = 'Please enter a message.';
        responseDiv.classList.remove('hidden');
        return;
    }
    
    responseDiv.className = 'mt-2 text-sm text-blue-600';
    responseDiv.textContent = 'Sending SMS...';
    responseDiv.classList.remove('hidden');
    
    // Send AJAX request
    fetch('{{ route("admin.admissions.send-sms-ajax", $admission->id) }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            responseDiv.className = 'mt-2 text-sm text-green-600';
    ...
    .catch(error => {
        responseDiv.className = 'mt-2 text-sm text-red-600';
        responseDiv.textContent = '✗ Error sending SMS. Please try again.';
