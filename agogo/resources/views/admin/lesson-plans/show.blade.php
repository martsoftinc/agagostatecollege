...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <a href="{{ route('admin.lesson-plans.index') }}"
       class="text-slate-500 hover:text-slate-800 text-sm font-semibold flex items-center space-x-1 w-fit">
      <i class="fa-solid fa-arrow-left"></i>
      <span>Back to Lesson Plans</span>
    </a>

    <div class="flex items-center space-x-2">
      <a href="{{ route('admin.lesson-plans.pdf', $lessonPlan) }}"
         class="px-3.5 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-semibold text-xs transition flex items-center space-x-1 border border-rose-200">
  ...
  <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
    ...
    <p class="text-xs text-slate-500 mt-1">
      Prepared by <strong>{{ $lessonPlan->author?->name ?? 'Unknown' }}</strong>
      on {{ $lessonPlan->lesson_date ? $lessonPlan->lesson_date->format('M d, Y') : $lessonPlan->created_at->format('M d, Y') }}
    </p>
  </div>
  ...
  <!-- Shared With (admin can see) -->
  @if($lessonPlan->sharedWithUsers->count() > 0)
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
  ...
  <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
    <h2 class="text-sm font-extrabold uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-2">
      Curriculum & Objectives
    </h2>
      ...
      <div class="space-y-3">
        @foreach($lessonPlan->resources as $resource)
          <a href="{{ $resource->url }}" target="_blank" rel="noopener"
             class="flex items-start space-x-3 p-3 rounded-xl border border-slate-200 hover:border-asc-green hover:bg-slate-50 transition group">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        @foreach($lessonPlan->attachments as $file)
          <a href="{{ Storage::disk($file->disk)->url($file->file_path) }}" target="_blank"
             class="flex items-center space-x-3 p-3 rounded-xl border border-slate-200 hover:border-asc-green hover:bg-slate-50 transition group">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0
              {{ $file->extension === 'pdf' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600' }}">
              <i class="fa-solid {{ $file->extension === 'pdf' ? 'fa-file-pdf' : 'fa-image' }} text-lg"></i>
            </div>
            <div class="min-w-0 flex-1">
